param(
  ...
  [Parameter(Mandatory)][datetime]$DestroyDeadline,
...
$planPath = Join-Path $planDirectory 'demo.tfplan'
...
$preflight = & (Join-Path $PSScriptRoot 'preflight.ps1') | ConvertFrom-Json
$cost = & (Join-Path $PSScriptRoot 'cost-estimate.ps1') -Hours $MaximumHours | ConvertFrom-Json
...
try {
  ...
  $planArgs = @('-input=false', '-refresh=false', '-no-color', "-out=$planPath")
  terraform "-chdir=$demo" plan @planArgs | Out-Null
  if ($LASTEXITCODE -ne 0) { throw 'terraform plan failed.' }

  $review = terraform "-chdir=$demo" show -json $planPath | python (Join-Path $PSScriptRoot 'review-plan.py') | ConvertFrom-Json
...
"@
  $summary | Set-Content -LiteralPath (Join-Path $root 'approval-summary.private.txt') -Encoding utf8
