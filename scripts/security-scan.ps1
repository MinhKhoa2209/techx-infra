...
trivy config --severity HIGH,CRITICAL --exit-code 1 --ignorefile (Join-Path $root '.trivyignore') --skip-dirs '.terraform,.terraform-helm,.plans' --skip-version-check $root
if ($LASTEXITCODE -ne 0) { throw 'Trivy configuration scan failed.' }
